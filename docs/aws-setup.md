...
補足： 管理用ユーザーが請求画面を見るには、rootで次の設定を有効にする。

1. rootでサインインする（この設定はrootだけが変更できる）
2. 右上のアカウントメニューから **アカウント** を開く
   - 直接開くなら <https://us-east-1.console.aws.amazon.com/billing/home#/account>
3. 下へスクロールし、「IAMユーザー/ロールによる請求情報へのアクセス」を探す
4. **編集** →「IAMアクセスのアクティブ化」にチェック → **更新**

見つからないときは、請求とコスト管理の左メニュー下部にある **アカウント設定** を開く。
...
3. 保存先のS3バケットは新規作成でよい。CloudTrailが `aws-cloudtrail-logs-<アカウントID>-<ランダム>` という名前で作り、必要なバケットポリシーも自動で入れる
4. ログファイルの検証を有効にする
5. イベントタイプは「管理イベント」だけでよい（データイベントは費用がかかる）
6. 作成後、**イベント履歴** で自分の操作が記録されるか確かめる
7. 証跡の画面に **配信された最後のログファイル** の時刻が出ていれば、書き込みできている

> S3のライフサイクルで、90日後にログを削除する設定にしている。
...
**2026-09-17に完了。** 本番は新しいアプリ `d3fj0jchd8ri0z` の `main` から配信している。古いアプリ `d1o1ui2gd5pshh` と `d1d0cu0fwxm76y` は削除した。以下は作業の記録。

新しいアプリは作成済み（`d3fj0jchd8ri0z`。2026-09-17）。GitHubのリポジトリーを `imrg-web-main` から `imrg-platform` へ移した（コミット履歴の個人のメールアドレスを消すため、新しいリポジトリーを作った）。今の本番のアプリ（`d1o1ui2gd5pshh`）は古いリポジトリーの `master` につながっている。
