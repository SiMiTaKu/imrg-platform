...
- [ ] **7-1. 条文データを公開の場から外す**（1〜2時間・最優先）
  - imrg-platformはPUBLIC。画面は閉じていても**ソースコードは誰でも読める**
  - `entities/rule/api/` を `~/imrg/imrg-hub/vendor/rulebook/2025/` へ退避し、リポジトリーから消す
  - 残すもの・捨てるものの一覧は [rules-guide.md](rules-guide.md) の9
