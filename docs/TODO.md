...
- [ ] **5-7. 規則について自然言語で質問できる AI（Bedrock）**
  ...
  - **2026-09-26: 許諾を取りに行かないと決めたので、規則の全文を渡す形はやめる。**
    渡すのは自分で書いた解説（`entities/ruleGuide/`）の本文にする。方針は [rules-guide.md](rules-guide.md)。
    解説の分量は条文より少ないので、プロンプトに入る前提は変わらない
  - 単価は実際に見積もるとき、AWSの料金ページで確認する（2026-09-21時点ではページから取れなかった）
- [ ] **5-8. 技の図を描き直す**（2026-09-23に後回し、2026-09-26に前提が変わった）
  - **2026-09-26: いまの棒人間は冊子の線画を写し取ったものなので、精度の問題ではなく使えない。**
    `STICK_SKELETONS`（座標）は捨て、**自分の撮った映像から描き直す**。
    仕組み（`StickPose`・コマ送り・`poseOf`・`sequenceOf`）は自作なので残す。
    詳細は [rules-guide.md](rules-guide.md) の8-2と優先7
  - 対象は3.6.3図解（冊子52〜60ページ・89技）と3.8.9難度表の図解（71〜76ページ・83技）の計172か所
  - 差し替えるときに捨てるのは `STICK_SKELETONS`（姿勢ごとの関節の位置）だけ。
    姿勢の種類（`StickPose`）・コマの並び（`STICK_SEQUENCES`）・技名からの選び方（`poseOf` `sequenceOf`）は
    そのまま使える作りにしてある
...
- [ ] **5-10. 採点の結果をXへ共有したときに、結果ごとの画像を出す**（2026-09-25に後回しと判断）
  - いまは結果の画面で「グラフを保存」してから、自分で投稿に添える形。
    Xのカード（`og:image`）は全員同じ `images/ogp.png` になる
  - 静的な書き出し（adapter-static → S3 + CloudFront）なので、
    リクエストごとにHTMLを作る場所が無い。`?score=8.50` を付けても返るHTMLは同じ
  - やるなら、共有用のURL（`/judge/result/?a=…&b=…`）だけをLambda Function URLで受け、
    値からOGタグと画像を組み立てて返す。固定費はほぼゼロで、かかるのは半日から1日ほど
  - 急がない理由。Xでは直接添えた画像のほうがカードより大きく出て伸びやすく、
    いまの「保存して添える」でも実用上は困らない
...
## Phase 7: ルール解説（`/rules/`）の作り直し（2026-09-26に方針決定）

**方針・決まりごと・作る順は [rules-guide.md](rules-guide.md) にまとめた。作業の前に必ず読む。**

条文をそのまま写した今の `/rules/` は捨て、**自分の言葉で書いた解説書**に置き換える。
きっかけは、協会の著作物を使う形は取れないという回答（2026-09-26）。
ただし作り直す理由は制約だけではなく、**いまの形では読者に届いていない**ことも大きい。

- [x] **7-1. 条文データを公開の場から外す**（2026-09-27）
  - imrg-platformはPUBLIC。画面は閉じていても**ソースコードは誰でも読める**状態だった
  - `entities/rule/` を丸ごと `~/imrg/imrg-hub/vendor/rulebook/2025/` へ退避し、リポジトリーから消した（51ファイル）。退避先には、使ってよい範囲を書いたREADMEを置いた
  - 冊子の見た目を再現していた部品（`RuleFigure` `RulePaperFrame` `RuleSource` `StickFigure` `ApparatusMark` `RulesPage`）も消した。`/rules/` は `ComingSoon` だけを出す
  - **文言も直した。** トップの特長カードが「規則集を読みやすく並べ直した」、そのバッジが**「全文」**になっていて、公開中の本番に出ていた。ほかに `meta`・構造化データ・審判ページの次の一手にも「規則集」「ルールブック」が入っていた。**8言語すべて**直した（一覧は [rules-guide.md](rules-guide.md) の3-2）
  - `PUBLIC_RULES_PUBLISHED` は消した。出すものが無くなったので、出すかどうかの判定が要らない
  - 書き出したファイルに条文が残っていないことを確かめた（条文由来の5語でどれも0件）
  - gitの履歴に残る分は、開いているPRを片付けてからあらためて判断する（同9-2）
- [x] **7-2. 骨組みと決まりごとを実装に落とす**（2026-09-27）
  - `entities/ruleGuide/` を作った。鍵の木（`GUIDE_KEY_TREE`）・本文のかたまり（`GuideBlock`）・ページ（`GuidePage`）
  - **断り書きは型で必須にした。** `GuidePage.disclaimer` は省略できないので、書き忘れると型で落ちる
  - **本文の決まりをテストで測るようにした**（`tests/unit/entities/ruleGuide/guideText.spec.ts`）。
    出してはいけない言葉（協会名・委員会・公認・推奨・ルールブック）、1文40字まで、ひらがなで書く言葉。
    textlintはMarkdownを見る道具なので、TypeScriptの中の文はテストで見る
  - **URLは本文を書いた時点で生える。** 木に足しただけでは生えない（`publishedGuideKeys`）。
    ルートは `/rules/[...guide]/` のひとつだけで、ページを増やしてもルーティングを触らなくてよい
  - **書いていないページへのリンクは出さない。** 絵（`figure`）も、まだ描いていないのでかたまりごと出さない。
    上へ戻る先は、本文のあるいちばん近い先祖まで飛ばす（`guideUpKey`）。
    ビルドのリンク検査が `/rules/group/` の404を見つけてくれたので直した
  - `ROUTES.rules` を `{ index, page }` にした。参照していた4か所を `.index` へ向けた
  - `META_DATA.ruleGuide()` を足した。説明文は本文の最初の答え（`lead`）から作るので、書き忘れが起きない
  - 文言は `messages/rule_guide/`（8言語）。`project.inlang/settings.json` に登録した
- [x] **7-3a. 最初の1ページ「隊形移動とは」**（2026-09-27）
  - `/rules/group/formation/`。2日で344表示・クリック0だった語をねらう
  - 誤記（体型移動・体形移動・体系移動）の言い換えを本文に入れ、体育祭の隊形移動との違いにも触れた
  - 出どころは `~/imrg/imrg-hub/knowledge/competition/group-composition.md`。規則集は見ていない
  - 既存の動画（`FerTYZ5I6CU`）をページに埋めた
- [ ] **7-3b. 残り2ページ**（各2〜3時間）
  - `/rules/toshu/`（徒手体操とは）、`/rules/score/`（点はどう決まるか）
- [ ] **7-4. 入口と残りの層を埋める**（8〜12時間）
- [ ] **7-5. 用語集 `/rules/words/`**（3〜4時間）
- [ ] **7-6. 公開と下回りの修正**（2〜3時間）
  - `PUBLIC_RULES_PUBLISHED` を外しnoindexを解除。あわせてviewport・canonical・ogを直す
  - **viewport の修正は1行で効く。7-6を待たず先に出してよい**
- [ ] **7-7. 図を入れる**（1枠0.5〜1時間）… 5-8と同じ作業
- [ ] **7-8. 8言語へ広げる**（1ページ0.5〜1時間 × 8言語）
  - **待ち：7-5 の完了。** 日本語を書き切ってから始める
- [ ] **7-9. ページから動画へ送る**（1本2時間）
  - **待ち：7-3 の完了。** 送り先ができてから作る
