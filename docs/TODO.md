...
- [ ] **7-2. 骨組みと決まりごとを実装に落とす**（4〜6時間）
  - `entities/ruleGuide/` の型、断り書きを型で必須にする仕掛け、禁止語のテスト、textlintのルール追加
  - URLの木から `ROUTES` とsitemapを作る（ページを足すと自動で載る形にする）
- [ ] **7-3. 検索需要の大きい3ページを書く**（各2〜3時間）
  - `/rules/group/formation/`（隊形移動とは）… 2日で344表示・クリック0を取りに行く
