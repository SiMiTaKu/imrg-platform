<script context='module' lang='ts'>
  ...
  const PERIODS: { key: EventPeriod; ja: string; en: string }[] = [
    { key: "upcoming", ja: "これから", en: "Upcoming" }, { key: "past", ja: "終わった", en: "Past" }, { key: "all", ja: "すべて", en: "All" },
  ]

  // カレンダーで移動できる範囲（データのある最初の月から最後の月まで）
  const MONTH_KEYS = EVENTS.flatMap((event) => [ toMonthKey(event.startDate), toMonthKey(event.endDate ?? event.startDate) ])
...
<article class='calendar'
  ...
  <section class='search'
           aria-label='検索と絞り込み / Search and filter'>
    <label class='keyword'>
  ...
  <div class='toolbar'>
    <div class='segmented'
         aria-label='表示の切り替え / View'
         role='group'>
      <button class='segment'
              class:active={state.view === "calendar"}
              type='button'
              aria-pressed={state.view === "calendar"}
              on:click={() => update({ view: "calendar" })}>カレンダー<span lang='en'>Calendar</span></button>
      <button class='segment'
      ...
      <div class='segmented'
        ...
        {#each PERIODS as period (period.key)}
          <button class='segment'
                  ...
                  on:click={() => update({ period: period.key })}>{period.ja}<span lang='en'>{period.en}</span></button>
        {/each}
      </div>
    {/if}
  ...
  {#if state.view === "calendar"}
    {#if state.keyword}
      <p class='hint'>
        ...
        <button class='link-button'
                type='button'
                on:click={() => update({ view: "list", period: "all" })}>リストで全部見る / See all in list</button>
      </p>
    {/if}

    <MonthCalendar canNext={state.month < LAST_MONTH}
    ...
    <section bind:this={resultsTop}
             class='results'>
      <div class='results-heading'>
        <h3 class='section-title'>
          {state.day ? `${formatDayJa(state.day)}のイベント` : `${formatMonthJa(state.month)}のイベント`}
          <span lang='en'>{state.day ? `Events on ${formatDayEn(state.day)}` : `Events in ${formatMonthEn(state.month)}`}</span>
        </h3>
        {#if state.day}
          <button class='link-button'
                  type='button'
                  on:click={() => update({ day: null })}>月のすべてを表示 / Show whole month</button>
        {/if}
      </div>
      ...
      {:else}
        <p class='empty'>この月のイベントは見つかりませんでした。<span lang='en'>No events this month.</span></p>
      {/if}
    </section>
  {:else}
    <section bind:this={resultsTop}
      ...
      {:else}
        <p class='empty'>条件に合うイベントは見つかりませんでした。<span lang='en'>No events match.</span></p>
      {/each}

      {#if slice.totalPages > 1}
        <Pagination page={slice.page}
