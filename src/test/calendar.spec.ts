...
describe("test filterEvents", () => {
  ...
  test("すべて：古い順に並べる", () => {
    expect(titles(filterEvents(events, { ...base, period: "all" }))).toEqual([
      "Z",
      "A",
      "B",
      "C",
...
describe("test groupByMonth", () => {
  test("開始月が続くかたまりごとにまとめる", () => {
    ...
    expect(result.map((group) => [ group.monthKey, group.events.length ])).toEqual([ [ "2026-10", 2 ], [ "2026-11", 1 ] ])
  })
})
...
describe("test eventsOnDay / eventsInMonth", () => {
  const events = [
    makeEvent({ titleJa: "期間", startDate: "2026-10-30", endDate: "2026-11-01" }), makeEvent({ titleJa: "1日", startDate: "2026-11-15" }), makeEvent({ titleJa: "年月だけ", startDate: "2026-11" }),
  ]
...
describe("test parseState / serializeState", () => {
  ...
  test("おかしな値は初期値にする", () => {
    const result = parseState("?view=grid&category=unknown&period=later&page=-2&month=2026-1&day=2026-12-01", "2026-09")
