...
export const GET: RequestHandler = async () => {
  ...
  const eventEntries = EVENTS.map((event) => ({ path: `/calendar/${event.id}`, lastmod: UPDATED_AT, priority: 0.6 }))
  const entries: SitemapEntry[] = STATIC_ENTRIES.concat(eventEntries)

  return new Response(buildSitemapXml(PUBLIC_BASE_URL, entries), { headers: { "Content-Type": "application/xml; charset=utf-8" } })
}
