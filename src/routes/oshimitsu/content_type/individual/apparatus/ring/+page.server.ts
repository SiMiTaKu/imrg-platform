...
export const load: PageServerLoad<OutputData> = async ({ locals }) => {
  const title = `推しミツ！（${ContentType.INDIVIDUAL.label} × ${Apparatus.RING.label}） | 男子新体操国際化プロジェクト ~日本の文化を世界のスポーツへ~`
  const description =
  ...
  return {
    ...
    layout: {
      ...
      nofollow: false,
      canonical: true,
      path: `/oshimitsu/content_type/${ContentType.INDIVIDUAL.slug}/apparatus/${Apparatus.RING.slug}`,
      ogp: {
        title,
        description,
        type: "website",
