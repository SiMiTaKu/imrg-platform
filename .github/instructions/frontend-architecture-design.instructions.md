...
- oshiage は多言語対応（i18n リソース）を必須にしているが、このサイトは**日本語と英語を同じページに併記**している（`WithEnglishHeading` など）。`/en/` を分けるかは TODO 6-4 で決める
- 英語の文言は `lang="en"` を付けた要素に入れる。画像の中に文字を入れず、翻訳できない UI を増やさない
- 日付・数値の表示は、日本語用と英語用の整形関数を分ける（例: カレンダーの `formatDateRangeJa` / `formatDateRangeEn`）
- 分けることになった場合に備えて、文言は部品の中に散らばらせず、`_data` や定数にまとめる
