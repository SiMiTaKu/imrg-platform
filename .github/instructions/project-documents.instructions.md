...
- [src/routes/oshimitsu/README_V1.md](../../src/routes/oshimitsu/README_V1.md) … 推しミツ！の要件（`README_V0.md` はブレスト時の記録）

## リポジトリ外の資料

- `~/imrg/calendar-data/README.md` … 大会カレンダーの元データ（JSON）と、`src/views/page/calendar/_data/events.ts` を生成するスクリプトの使い方。`events.ts` は手で編集しない（TODO 6-1 で取り込み予定）
